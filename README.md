...
**総合的な示唆**  
グレーゾーン（年齢・婚姻状況）を除いて図を見ると、コントロール内の要因は収入に対してプラスにもマイナスにも作用する一方、コントロール外の要因はマイナスにしか作用しないことがわかる。これは、マイノリティ属性を持つ人々はそもそも不利な条件でスタート地点に立つことを示唆している。また、以下の点には留意が必要である。
