...
| **グレーゾーン** | `age`（年齢）| 時間経過は選択不可だが平等に作用する※ |
| **グレーゾーン** | `marital_status`（婚姻状況）| 選択だが文化・経済状況にも依存 |

※ 年齢は誰もが等しく経験するものであり、`sex`や`race`のような不平等の構造的源泉とは性質が異なる。本分析では、年齢を「コントロール外だが中立的な背景変数」として位置づけた。

### 考察

![Factor Analysis](images/factor_analysis.png)
*図10: 各変数の高収入オッズへの影響（オッズ比・対数スケール表示）。オレンジがコントロール外、青がコントロール内、グレーがグレーゾーン。表示変数はp < 0.05 かつ OR < 0.6 または OR > 1.67（対数スケール上で対称なカットオフ）を満たすものを機械的に選定した。OR > 1.67 は OR < 0.6 の逆数であり、プラス・マイナス方向で等しい効果量の基準を意味する。*

オッズ比・特徴量重要度の結果を踏まえると、以下のことが言える。

**コントロール外の要因について**  
性別については女性が男性より OR=0.48 と負の影響を示す（高収入のオッズが約52%低い）。出身国（Vietnam: OR=0.27、South: OR=0.29、China: OR=0.41）や人種（Amer-Indian-Eskimo: OR=0.57）も同様に負の影響を示した。コントロール外の要因は、図10の選定基準（OR < 0.6 または OR > 1.67）を満たすものが全てマイナス方向（OR < 1）に集中している。

**グレーゾーンの要因について**  
年齢（50〜59: OR=4.37、40〜49: OR=4.02）は収入に強い正の影響を持つ。婚姻状況については、既婚・配偶者同居を基準とすると未婚（Never-married: OR=0.08）が特に大きな負の影響を示しており、本モデルで最も効果量が大きい変数の一つである。

**コントロール内の要因について**  
学歴・職業・労働時間はいずれも収入と強い関連を示し、特に学歴（High: OR=2.85、Low: OR=0.38）と職業（Exec-managerial: OR=2.25、Farming-fishing: OR=0.43）の影響は大きい。コントロール内の要因はプラス・マイナス両方向に作用しており、選択によって高収入にも低収入にも傾き得ることを示している。
