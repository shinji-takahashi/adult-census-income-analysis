...
**性別（基準: 男性）**
- 女性（Female）: **−5%**
> ※ `relationship: Wife` の +10% と `sex: Female` の −5% は一見矛盾するが、これは比較対象の違いによる。前者は既婚かつ就労している女性に限定した比較であり、当時（1994年）の米国において就労を継続している既婚女性はキャリア志向の選択集団である可能性が高い。女性全体の平均的なペナルティ（−5%）とは異なる集団を捉えていることに留意が必要である。
...
![サブグループ分析](images/subgroup_analysis_married.png)
...
*図7: 各変数の高収入オッズへの影響（%）。オレンジがコントロール外、青がコントロール内、グレーがグレーゾーン。*
